...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        # return sig * (sig * (-1) + 1.0) * grad_output
        return grad_output.f.mul_zip(
            grad_output, sig.f.mul_zip(sig, 1 + sig.f.neg_map(sig))
        )
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        # TODO: Implement for Task 2.3.
        # raise NotImplementedError("Need to implement for Task 2.3")
        s = order._tensor._storage
        o = [int(x) for x in s]
        ctx.save_for_backward(*o)
        return minitorch.Tensor(a._tensor.permute(*o), backend=a.backend)
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        original_order = [order.index(i) for i in range(len(order))]
        return (
            minitorch.Tensor(
